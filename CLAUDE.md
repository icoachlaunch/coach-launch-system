...
| Cash Flow Engine™ funnel pages (Step 5) | `modules/2-money-magnet/cash-flow-engine/SKILL.md` — the Funnel Builder. It reads the client's `BRAND KIT:` block and skins the phase templates in it (RULE 4: the Kit is the only look source). The 19 templates under `Funnel-templates/PHASE-*/` are structural references and still carry their own hex — the skill does the skinning. See memory `cash-flow-engine-funnel-approach` |
